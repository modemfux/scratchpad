# Scratchpad and notes

Заметки, инструкции и т.п.

- [**Base linux**](/01_linux_base/01_linux_base_onefile.md)
  - [Изменение hostname](/01_linux_base/hostname.md)
  - [Разрешаем SUDO без пароля](/01_linux_base/sudo_passwordless.md)
  - [Изменить редактор по умолчанию](/01_linux_base/default_text_editor.md)
  - [Информация о sysctl](/01_linux_base/sysctl.md)
- [**Network**](/02_linux_network/02_linux_network_onefile.md)
  - [Создание dot1q сабинтерфейса](/02_linux_network/create_subinterface.md)
  - [Создание bridge](/02_linux_network/create_bridge.md)
  - [Новый loopback-интерфейс](/02_linux_network/create_new_loopback.md)
  - [VRF](/02_linux_network/vrf.md)
  - [Включение работы MPLS](/02_linux_network/enable_mpls.md)
  - [Включение работы VxLAN](/02_linux_network/enable_vxlan.md)
  - [Настройка интерфейсов через netplan на Ubuntu](/02_linux_network/ubuntu_netplan.md)
  - [Настройка интерфейсов через network.interfaces](/02_linux_network/network_interfaces.md)
  - [Изменить наименование сетевых интерфейсов на ethX](/02_linux_network/eth_like_interfaces_names.md)
  - [LLDP](/02_linux_network/lldp.md)
- [**Docker**](/03_docker/03_docker_onefile.md)
